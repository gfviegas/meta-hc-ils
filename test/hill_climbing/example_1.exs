...
{:ok, file} = File.open("./runs/example_1_#{DateTime.now!("Etc/UTC") |> DateTime.to_iso8601}.log", [:append])
...
options = [
  ...
  variables: [
    %Variable{
      name: :x,
      constraint: %Constraint{lower_boundary: -1.5, higher_boundary: 4}
    },
    %Variable{
      name: :y,
      constraint: %Constraint{lower_boundary: -3, higher_boundary: 4}
    }
  ],
  noise_size: 0.05,
  max_iterations: 50,
